...
const SignsScreen = ({ route, navigation }) => {
    ...
    useEffect(() => {
        fetch(`http://192.168.0.103:5000/api/signs/${encodeURIComponent(category)}`)
            .then((response) => {
                if (!response.ok) {
    ...
    renderItem={({ item }) => {
        const imageUrl = `http://192.168.0.103:5000/api/image/${encodeURIComponent(category)}/${encodeURIComponent(item.name)}.png`;
